package org.statnlp.targetedsentiment.f.baseline;
		...
		if (ids_parent[4] == NodeType.Span.ordinal()) // !input_token[pos_parent].getName().startsWith("O")
		{
			...
			if (featureArr.size() > 0) {
				...
				for (String[] item : featureArr) {
					...
					feature.add(new String[] { item[0] + item[2], Sent, "_link(NE,SENT)" });

					/*
					 * if (item[0].contains("_sent_") ||
					 * item[0].contains("_id_")) {
					 * 
					 * //feature.add(new String[]{item[0] + item[2], "[" + Sent
					 * + "]", "_sent(SENT)" }); //System.out.println(item[0] +
					 * "|[" + Sent + "]|" + item[2] + "_sent(SENT)" );
					 * //feature.add(new String[]{item[0] + item[2], "[" + NE +
					 * "_VOLITIONAL," + Sent + "]", "_link(NE,SENT)"});
					 * //System.out.println(item[0] + "|[" + NE + "_VOLITIONAL,"
					 * + Sent + "]|" + item[2] + "_link(NE,SENT)"); }
					 */
				}

				// .add(new String[]{"sent", "[" + NE + "_VOLITIONAL," + Sent +
				// "]", "_link(NE,SENT)"});
				// System.out.println("========================");
