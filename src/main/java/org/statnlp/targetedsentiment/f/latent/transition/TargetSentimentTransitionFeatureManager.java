...
public class TargetSentimentTransitionFeatureManager extends FeatureManager {

	private NeuralNetworkCore net;
	public String neuralType;
	public boolean moreBinaryFeatures = false;

	public TargetSentimentTransitionFeatureManager(GlobalNetworkParam param_g) {
		super(param_g, null);
	...
	public TargetSentimentTransitionFeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
		super(param_g);
		if (NetworkConfig.USE_NEURAL_FEATURES) {
			this.net = (NeuralNetworkCore) param_g.getNNParamG().getNet(0);
		}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		if (TargetSentimentGlobal.ALLOW_NULL_TARGET && inst.numNULLTarget > 0) {
			if (ids_parent[4] == NodeType.Start.ordinal()) {

				long nullNode = network.getNode(children_k[1]);

				int[] ids_nullNode = NetworkIDMapper.toHybridNodeArray(nullNode);
				int polar_nullNode = PolarityTypeSize - ids_nullNode[1];
				String sent = PolarityType.values()[polar_nullNode].name();
				// System.out.println("NULL Target Sent:" + sent);

				feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "NULL_sent_:", sent, ""));

				for (int pos = 0; pos < size; pos++) {
					String[] current = inputs.get(pos);

					String w = getToken(inputs, pos, 0);
					String nw = getToken(inputs, pos, 0);

					String _sent_ = getToken(inputs, pos, TSInstance.WORD_FEATURE_TYPES._sent_.ordinal());

					// if (!_sent_.equals("_"))
					{
						feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment", sent, "current_word:" + w));
						feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Bigram + "-" + "O_hidden_sentiment", sent, w + "-" + nw));
					}
				}
			}
		...
		boolean parentIsSpanNode = (node_parent == NodeType.Span.ordinal());
		boolean childIsSpanNode = (node_child == NodeType.Span.ordinal());

		// left to left, right to right, left to entity
		if ((subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("B") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("B"))
				|| (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("A") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("A")) || (subnode_child == SubNodeType.e.ordinal())) {
			...
			/*
			 * feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Bigram
			 * + "-" + word_feature_type , Out, prev_feature + "|||" +
			 * current_feature )); feature.add(this._param_g.toFeature(network,
			 * FEATURE_TYPES.Trigram + "-" + word_feature_type , Out,
			 * prev_feature + "|||" + current_feature + "|||" + next_feature ));
			 * feature.add(this._param_g.toFeature(network,
			 * FEATURE_TYPES.Fourgram + "-" + word_feature_type , Out,
...

			{
				...
				// word embedding
				// feature.add(this._param_g.toFeature(network,
				// "<WordEmbeddingonWord>", Out, current.getName()));
				 ...
				 * 
				 * prefix = "_prev"; for (String[] f :
				 * mynetwork.horizon_feature[i]) { addPrefix(featureArr, f,
				 * prefix, Sent, NE); }
				 * 
				 * }
				 */
				...
				if (featureArr.size() > 0) {
					...
					for (String[] item : featureArr) {
						...
						/*
						 * if ( item[0].contains("_id_"))
						 * //item[0].contains("_sent_") || {
						 * 
						 * //feature.add(new String[]{item[0] + item[2], "[" +
						 * Sent + "]", "_sent(SENT)" }); //feature.add(new
						 * String[]{item[0] + item[2], "[" + NE + "_VOLITIONAL,"
						 * + Sent + "]", "_link(NE,SENT)"});
						 * //feature.add(this._param_g.toFeature(network,
						 * item[0] + item[2], "[" + Sent + "]", "_sent(SENT)"));
						 * //feature.add(this._param_g.toFeature(network,
						 * item[0] + item[2], "[" + NE + "_VOLITIONAL," + Sent +
						 * "]", "_link(NE,SENT)"));
						 * 
						 * 
						 * }
					...
					// feature.add(new String[]{"sent", "[" + NE +
					// "_VOLITIONAL," + Sent + "]", "_link(NE,SENT)"});
					// feature.add(this._param_g.toFeature(network, "sent", "["
					// + NE + "_VOLITIONAL," + Sent + "]", "_link(NE,SENT)"));
		...
		if (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("B") && subnode_child == SubNodeType.e.ordinal()) {
		...
		} else if (subnode_parent == SubNodeType.e.ordinal() && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("A")) {
		...
		if (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("B") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("B")) {
			//if (!inputs.get(pos_parent)[5].equals("_"))
				WordEmbeddingFeature = word_parent;

			if (!TargetSentimentGlobal.WITHOUT_HIDDEN_SENTIMENT) {

				if (TargetSentimentGlobal.INCLUDE_FEATURE_AFTER_OR_BEFORE) {
					feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment", "_After_or_Before", "current_word:" + word_parent));
				}
				String featureName = "_Before";
				if (TargetSentimentGlobal.INCLUDE_FEATURE_ORIG) {
					feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment", featureName, "current_word:" + word_parent));
				}
				if (TargetSentimentGlobal.INCLUDE_FEATURE_ORIG_WITH_POLARITY) {
					feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment", PolarityType.values()[polar_parent].name() + featureName, "current_word:" + word_parent));
				}

				int BIdx = subnode_parent - SubNodeType.Bpp.ordinal();
				if (BIdx / 3 == BIdx % 3) {
					featureName = "_Same";
				} else {
					featureName = "_Diff";
				...
				String transitionType = "_" + SubNodeType.values()[subnode_parent].name().substring(1);

				if (TargetSentimentGlobal.INCLUDE_FEATURE_SPLIT) {
					feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment", featureName, "current_word:" + word_parent));
					// feature.add(this._param_g.toFeature(network,
					// FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment" ,
					// transitionType, "current_word:" +word_parent ));
				}
				if (TargetSentimentGlobal.INCLUDE_FEATURE_SPLIT_WITH_POLARITY) {
					feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment", PolarityType.values()[polar_parent].name() + featureName, "current_word:" + word_parent));
					// feature.add(this._param_g.toFeature(network,
					// FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment" ,
					// PolarityType.values()[polar_parent].name()+transitionType,
					// "current_word:" +word_parent ));
				}
				...
				// feature.add(this._param_g.toFeature(network,
				// FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment" ,
				// PolarityType.values()[polar_parent].name(), "current_word:"
				// +word_parent + "|||" + "next_word:" +next_feature ));
		...
		} else if (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("A") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("A")) {
			//if (!inputs.get(pos_parent)[5].equals("_"))
				WordEmbeddingFeature = word_parent;
			...
			if (!TargetSentimentGlobal.WITHOUT_HIDDEN_SENTIMENT) {
				...
				int AIdx = subnode_parent - SubNodeType.App.ordinal();
				if (AIdx / 3 == AIdx % 3) {
					featureName = "_Same";
				} else {
					featureName = "_Diff";
		...
		} else if (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("A") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("B")) {
			// feature.add(this._param_g.toFeature(network,
			// FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment_change" ,
			// PolarityType.values()[polar_child].name(),
			// PolarityType.values()[polar_parent].name() ));
			// feature.add(this._param_g.toFeature(network,
			// FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment_change" ,
			// PolarityType.values()[polar_parent].name(),PolarityType.values()[polar_child].name()
			// ));
			// feature.add(this._param_g.toFeature(network,
			// FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment_change" ,
			// PolarityType.values()[polar_parent].name() + "|||" +
			// PolarityType.values()[polar_child].name(), word_parent + "|||" +
			// next_feature ));
		...
		if (feature.size() > 0) {
			...
			if (NetworkConfig.USE_NEURAL_FEATURES) {
				...
				if (neuralType.startsWith("lstm")) {
					...
					if (subnode_child == SubNodeType.e.ordinal()) {
						if (SubNodeType.values()[subnode_parent].name().startsWith("B")) { // B
							entityId = 1;
						...
						if (SubNodeType.values()[subnode_parent].name().startsWith("B") && SubNodeType.values()[subnode_child].name().startsWith("B")) {
							entityId = 0;
							input = new SimpleImmutableEntry<String, Integer>(inst.getSentence(), pos_parent);
						} else if (SubNodeType.values()[subnode_parent].name().startsWith("A") && SubNodeType.values()[subnode_child].name().startsWith("A")) {
							input = new SimpleImmutableEntry<String, Integer>(inst.getSentence(), pos_child);
						}
					}
					...
					// this.addNeural(network, 1, parent_k, children_k_index,
					// input, null, sentId);
				}
				...
				// }
					...
					if (WordEmbeddingFeature != null) {
						WordEmbeddingFeature = WordEmbeddingFeature.toLowerCase();
						
						this.addNeural(network, 0, parent_k, children_k_index, WordEmbeddingFeature, sentId);
					}
				} else {
	 ...
	 * feature.add(this.getParam_G().toFeature(network,
	...
	public String clean(String s) {
		...
		/*
		 * else if (s.startsWith("@")) { str = "<USERNAME>"; }
		 */
		else {
