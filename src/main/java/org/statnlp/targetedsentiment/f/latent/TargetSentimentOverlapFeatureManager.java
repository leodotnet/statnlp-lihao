...
import org.statnlp.targetedsentiment.f.latent.TargetSentimentOverlapCompiler.*;

public class TargetSentimentOverlapFeatureManager extends FeatureManager {
	
	private NeuralNetworkCore net;
	public String neuralType;
	public boolean moreBinaryFeatures = false;
	
	public TargetSentimentOverlapFeatureManager(GlobalNetworkParam param_g) {
		super(param_g, null);
	...
	public TargetSentimentOverlapFeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
		super(param_g);
		if (NetworkConfig.USE_NEURAL_FEATURES) {
			this.net = (NeuralNetworkCore) param_g.getNNParamG().getNet(0);
		}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		if ((subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("B") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("B"))
				|| (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("A") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("A")) 
		...
		if (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("B") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("B"))
		{
			if (!TargetSentimentGlobal.WITHOUT_HIDDEN_SENTIMENT)
			{
				...
				if(subnode_parent == SubNodeType.BS.ordinal()){
					featureName = "_Same";
				} else {
					featureName = "_Diff";
				}
		...
		else if (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("A") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("A"))
		{
			if (!TargetSentimentGlobal.WITHOUT_HIDDEN_SENTIMENT)
			{
				...
				if(subnode_parent == SubNodeType.AS.ordinal()){
					featureName = "_Same";
				} else {
					featureName = "_Diff";
				}
		...
		} else if (subnode_parent < SubNodeTypeSize && SubNodeType.values()[subnode_parent].name().startsWith("A") && subnode_child < SubNodeTypeSize && SubNodeType.values()[subnode_child].name().startsWith("B"))
		{
			...
			//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment_change" , PolarityType.values()[polar_parent].name() + "|||" + PolarityType.values()[polar_child].name(),  word_parent + "|||" + next_feature ));
			...
			if (TargetSentimentGlobal.ENABLE_WORD_EMBEDDING)
			{
				...
				if(NetworkConfig.USE_NEURAL_FEATURES && WordEmbeddingFeature != null){
					...
					if(neuralType.startsWith("lstm")) {
						...
						if (subnode_child == SubNodeType.e.ordinal()) {
							if (subnode_parent == SubNodeType.BS.ordinal()||subnode_parent == SubNodeType.BD.ordinal()) { //B
								entityId = 1;
							...
							if ((subnode_parent == SubNodeType.BS.ordinal() || subnode_parent == SubNodeType.BD.ordinal()) && (subnode_child == SubNodeType.BS.ordinal() || subnode_child == SubNodeType.BD.ordinal())) {
								entityId = 0;
								input = new SimpleImmutableEntry<String, Integer>(inst.getSentence(), pos_parent);
							} else if  ((subnode_parent == SubNodeType.AS.ordinal() || subnode_parent == SubNodeType.AD.ordinal()) && (subnode_child == SubNodeType.AS.ordinal()||subnode_child == SubNodeType.AD.ordinal())) {
								input = new SimpleImmutableEntry<String, Integer>(inst.getSentence(), pos_child);
							}
						}
