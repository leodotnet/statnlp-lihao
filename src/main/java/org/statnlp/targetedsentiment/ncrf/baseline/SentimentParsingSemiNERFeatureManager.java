...
import org.statnlp.targetedsentiment.ncrf.baseline.SentimentParsingSemiNERCompiler.*;

public class SentimentParsingSemiNERFeatureManager extends FeatureManager {
	...
	public SentimentParsingSemiNERFeatureManager(GlobalNetworkParam param_g) {
		super(param_g, null);
	...
	public SentimentParsingSemiNERFeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
		super(param_g);
		if (NetworkConfig.USE_NEURAL_FEATURES) {
			this.net = (NeuralNetworkCore) param_g.getNNParamG().getNet(0);
		}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		int subnode_parent = ids_parent[2];
		int subnode_child = ids_child[2];
		...
		if (subnode_parent == NodeType.B.ordinal() 
				&& (subnode_child == NodeType.O.ordinal() || subnode_child == NodeType.B.ordinal() || subnode_child == NodeType.X.ordinal() )) {
		...
		else if (subnode_parent == NodeType.O.ordinal() 
				&& (subnode_child == NodeType.O.ordinal() || subnode_child == NodeType.B.ordinal() || subnode_child == NodeType.X.ordinal() )) {
			...
			{
				curr_word = getToken(inputs, pos, 0);
				prev_word = getToken(inputs, pos - 1, 0);
				next_word = getToken(inputs, pos + 1, 0);
			
				
			
				feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O", "", "current_word:" + curr_word));
				
				//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O", "", "current_word:" + curr_word + "|||" + "next_word:" + next_word));
				//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O", "", "last_word:" + prev_word + "|||" + "current_word:" + curr_word));
				//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O", "", "last_word:" + prev_word + "|||" + "current_word:" + curr_word + "|||" + "next_word:" + next_word));
