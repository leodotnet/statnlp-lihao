...
public class SentimentParsingSemiFeatureManager extends FeatureManager {
	...
	public SentimentParsingSemiFeatureManager(GlobalNetworkParam param_g) {
		super(param_g, null);
	...
	public SentimentParsingSemiFeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
		super(param_g);
		if (NetworkConfig.USE_NEURAL_FEATURES) {
			this.net = (NeuralNetworkCore) param_g.getNNParamG().getNet(0);
		}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		if (subnode_parent == NodeType.B.ordinal() 
			...
			/****** NER End ******/
			curr_word = getToken(inputs, pos_child - 1, 0);
			prev_word = getToken(inputs, pos_child  -1 - 1, 0);
			next_word = getToken(inputs, pos_child - 1 + 1, 0);
