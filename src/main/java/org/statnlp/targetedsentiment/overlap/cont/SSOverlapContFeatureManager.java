...
public class SSOverlapContFeatureManager extends FeatureManager {

	private NeuralNetworkCore net;
	public String neuralType;
	public boolean moreBinaryFeatures = false;

	public SSOverlapContFeatureManager(GlobalNetworkParam param_g) {
		super(param_g, null);
	...
	public SSOverlapContFeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
		super(param_g);
		if (NetworkConfig.USE_NEURAL_FEATURES) {
			this.net = (NeuralNetworkCore) param_g.getNNParamG().getNet(0);
		}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		if (subnode_parent == NodeType.B0.ordinal()) {
			pos_parent = ids_parent[0] - 1;
		}

		if (subnode_child == NodeType.B0.ordinal()) {
			pos_child = ids_parent[0] - 1;
		}
		...
		if ((subnode_parent == NodeType.B.ordinal() && subnode_child == NodeType.B.ordinal()) || (subnode_parent == NodeType.A.ordinal() && subnode_child == NodeType.A.ordinal()) || (subnode_child == NodeType.E.ordinal())) {
			// word level feature for each position

			if (subnode_child == NodeType.E.ordinal()) {
...

			{
				...
				if (subnode_parent == NodeType.B.ordinal() && subnode_child == NodeType.E.ordinal()) {
					WordEmbeddingFeature = current[0];

					if (TargetSentimentGlobal.USE_PREPROCESS_SCRIPT)
						WordEmbeddingFeature = current[9];

				} else if (subnode_parent == NodeType.E.ordinal() && subnode_child == NodeType.A.ordinal()) {
					WordEmbeddingFeature = current[0];
		...
		if (TargetSentimentGlobal.OVERLAPPING_FEATURES) {
			if (subnode_parent == NodeType.B0.ordinal()) {
				String POSTag = inputs.get(pos_parent)[WORD_FEATURE_TYPES.postag.ordinal()];
				String ther_sentiment_polarity = inputs.get(pos_parent)[WORD_FEATURE_TYPES._sent_ther_.ordinal()];
				String polar = PolarityType.values()[polar_parent].name();
				if (!ther_sentiment_polarity.equals("_")) {
					ther_sentiment_polarity = ther_sentiment_polarity.split(":")[1];
				}
				
				//if (POSTag.startsWith("N") || POSTag.startsWith("V") || POSTag.startsWith("A")) 
				{
					//if (!ther_sentiment_polarity.equals("_")) 
					{
						//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "Overlapping:", PolarityType.values()[polar_parent].name(), "current_word:" + word_parent));
						//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "Overlapping-before:", PolarityType.values()[polar_parent].name(), "current_word:" + word_parent));
						feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment" ,polar, "current_word:" +word_parent ));
						//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment-before" ,PolarityType.values()[polar_parent].name(), "current_word:" +word_parent ));
					}
					
					if (ther_sentiment_polarity.contains(polar)) {
						feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment" , "matched", "" ));
					}
				}
			}

			if (subnode_parent == NodeType.A0.ordinal()) {
				String POSTag = inputs.get(pos_parent)[WORD_FEATURE_TYPES.postag.ordinal()];
				String ther_sentiment_polarity = inputs.get(pos_parent)[WORD_FEATURE_TYPES._sent_ther_.ordinal()];
				String polar = PolarityType.values()[polar_parent].name();
				if (!ther_sentiment_polarity.equals("_")) {
				...
				//if (POSTag.startsWith("N") || POSTag.startsWith("V") || POSTag.startsWith("A")) 
				{
					...
					{
						//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "Overlapping:", PolarityType.values()[polar_parent].name(), "current_word:" + word_parent));
						//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "Overlapping-after:", PolarityType.values()[polar_parent].name(), "current_word:" + word_parent));
						feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment" ,polar, "current_word:" +word_parent ));
						//feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment-after" ,PolarityType.values()[polar_parent].name(), "current_word:" +word_parent ));
					}
		...
		if (subnode_parent == NodeType.B.ordinal() && subnode_child == NodeType.E.ordinal()) {
			...
			// "current_ther_sentiment_polarity:" +
			// ther_sentiment_polarity[1]));

		} else if (subnode_parent == NodeType.E.ordinal() && subnode_child == NodeType.E.ordinal()) {
			...
			// feature.add(this._param_g.toFeature(network,
			// FEATURE_TYPES.Unigram + "-" + "NE_sentiment" , "",
			// PolarityType.values()[polar_parent].name() ));
			feature.add(this._param_g.toFeature(network, FEATURE_TYPES.Unigram + "-" + "NE_contains", PolarityType.values()[polar_parent].name(), "current_word:" + word_parent + "first_letter_upper:" + Character.isUpperCase(word_parent.charAt(0))));
		...
		if (subnode_parent == NodeType.B.ordinal() && subnode_child == NodeType.B.ordinal()) {
		...
		} else if (subnode_parent == NodeType.A.ordinal() && subnode_child == NodeType.A.ordinal()) {
			if (!TargetSentimentGlobal.WITHOUT_HIDDEN_SENTIMENT) {
		...
		} else if (subnode_parent == NodeType.A.ordinal() && subnode_child == NodeType.B.ordinal()) {
			// feature.add(this._param_g.toFeature(network,
			...
			// ));
			// feature.add(this._param_g.toFeature(network,
			// FEATURE_TYPES.Unigram + "-" + "O_hidden_sentiment_transition" ,
			// PolarityType.values()[polar_parent].name() + "|||" +
