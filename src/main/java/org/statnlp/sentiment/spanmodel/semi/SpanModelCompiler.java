package org.statnlp.sentiment.spanmodel.semi;
...
public class SpanModelCompiler extends SpanModelSuperCompiler {
	...
	public SentimentInstance decompile(Network network) {
		...
		ArrayList<int[]> pred_textspan_reverse = new ArrayList<int[]>();
		...
		while(true){
			...
			int[] textspan = new int[]{pos - 1, last_pos - 2};
			...
			pred_textspan_reverse.add(textspan);
			last_pos = pos;
		}
		...
		inst.textSpanList.clear();
		for(int i = pred_textspan_reverse.size() - 1; i >= 0 ; i--)
		{
			inst.textSpanList.add(pred_textspan_reverse.get(i));
		}
	...
	private long toNode_OP(int pos, int OPIndex ,int size){
		return NetworkIDMapper.toHybridNodeID(new int[]{pos + 1, OPIndex, 0, 0, NodeType.OP.ordinal()});
	}
	...
	public long[][] buildCommonNetwork(NetworkBuilder<BaseNetwork> lcrfNetwork, SentimentInstance inst, boolean labeled)
	{
		...
		for (int pos = 0; pos <= size; pos++) {
			...
			if (pos < size) 
			//for(int L = 0; L < SpanModelGlobal.SemiL && pos + L + 1 < size; L++)
			{
				//int[] textSpanIdx = new int[]{pos, pos + L + 1};
				for (int j = 0; j < OPNodeTypeSize; j++) {
					long node = this.toNode_OP(pos, j, size);
		...
		{
			lcrfNetwork.addEdge(node_array[0][i],  new long[]{leaf});
			sentimentStateCandidate.add(i); //SentimentStateType.neutral.ordinal());
		}
		...
		for(int OPPolar : new int[]{1, -1})
		{
			...
			for(Integer j : sentimentStateCandidate)
			{
				...
				for (int k = 0; k < NormalOPNodeTypeSize; k++) {
					...
					long edgeFrom = node_array[pos + L + 1][toNextSentimentState];
			...
			else
			{
				...
				for(Integer j : sentimentStateCandidate)
				{
					...
					long edgeFrom = node_array[pos + L + 1][i];
