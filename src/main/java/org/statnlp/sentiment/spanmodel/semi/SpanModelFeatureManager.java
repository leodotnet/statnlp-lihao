...
import org.statnlp.sentiment.spanmodel.semi.SpanModelCompiler.NodeType;
import org.statnlp.sentiment.spanmodel.semi.SpanModelCompiler.OPNodeType;
import org.statnlp.sentiment.spanmodel.semi.SpanModelCompiler.SentimentStateType;
...
public class SpanModelFeatureManager extends SpanModelSuperFeatureManager {
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		else
		{
			...
			int wordIdxFrom = pos_child_sentiment;
			int wordIdxTo = pos_parent;
			
			pos_lstm = wordIdxTo - 1;
			int pos_sentimentword = wordIdxTo - 1;
			...
			String sentimentWord = inputs.get(pos_sentimentword - 1).toLowerCase();
			
			feature.add(this._param_g.toFeature(network, "SentimentWord:" , OPFormSuffix,  sentimentWord));
			
			
			for(int i = wordIdxFrom; i < wordIdxTo - 1; i++)
			{
				String word = inputs.get(i - 1).toLowerCase();
