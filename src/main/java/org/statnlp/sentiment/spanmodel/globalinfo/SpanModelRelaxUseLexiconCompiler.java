...
public class SpanModelRelaxUseLexiconCompiler extends SpanModelSuperCompiler {
	...
	public SpanModelRelaxUseLexiconCompiler() {
		setCapacity();
	}

	public SpanModelRelaxUseLexiconCompiler(SentimentDict dict) {
		setCapacity();
	...
	public long[][][] buildCommonNetwork(NetworkBuilder<BaseNetwork> lcrfNetwork, SentimentInstance inst, boolean labeled) {
		...
		for (Integer sent : sents) {
			...
			for (int pos = 0; pos < size; pos++) {
				...
				int numIntensity = 0;
				boolean lastSpanNoSentimentWord = inst.sentimentWordList.size() < inst.textSpanList.size();
				int OPPolar = -1;
					...
					for (Integer j : sentimentStateCandidate) {
						...
						for (int k = 0; k < NormalOPNodeTypeSize; k++) {
							...
							if (pos < size - 1) {
								...
								if (k == OPNodeType.Magnify_pos.ordinal() || k == OPNodeType.Magnify_neg.ordinal()) {
									if (numIntensity == 0)
										continue;
								}
