package org.statnlp.sentiment.spanmodel.globalinfo.semi;
...
import org.statnlp.sentiment.spanmodel.globalinfo.SpanModelCompiler.NodeType;
		...
		else
		{
			...
			for(int ngram = 1; ngram <= NGRAM; ngram++)
			{
				for(int i = 0; i + ngram - 1 < words.size(); i++)
				{
					...
					feature.add(this._param_g.toFeature(network, "NGRAM_" + ngram + ":" , OPForm + "-" + sentimentState_child_Form + "-" + sentimentState_parent_Form + "|globalsent:" + globalSent,  featureForm));
			...
			for(int i = 0; i < words.size(); i++)
			{
				...
				feature.add(this._param_g.toFeature(network, "NGRAM_1*" + ":" , OPFormPrefix + "|globalsent:" + globalSent,  featureForm));
				...
				feature.add(this._param_g.toFeature(network, "NGRAM_1*" +  ":" , OPForm + "-" + sentimentState_child_Form + "-" + sentimentState_parent_Form + "|globalsent:" + globalSent,  featureForm));
		...
		if (feature.size() > 0)
		{
			if (SpanModelGlobal.ENABLE_WORD_EMBEDDING)
			{
				...
				if(NetworkConfig.USE_NEURAL_FEATURES && pos_lstm != -1) // && WordEmbeddingFeature != null)
				{
					...
					//this.addNeural(network, 0, parent_k, children_k_index, input, null, eId);
				} 
			}
