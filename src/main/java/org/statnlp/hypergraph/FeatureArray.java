...
public class FeatureArray implements Serializable{
	...
	public double getScore(LocalNetworkParam param, int version){
		...
		FeatureArray current = this;
		this._totalScore = 0.0;
		if (current != null){
			this._totalScore += this.getScoreHelper(param, version, current);
	...
	public double getScoreHelper(LocalNetworkParam param, int version, FeatureArray fa) {
		
		if(fa == NEGATIVE_INFINITY){
			return fa._totalScore;
		}

		if(!fa._isLocal != param.isGlobalMode()) {
			System.err.println(fa._next);
			throw new RuntimeException("This FeatureArray is local? "+fa._isLocal+"; The param is "+param.isGlobalMode());
		}

		//if the score is negative infinity, it means disabled.
		if(fa._totalScore == Double.NEGATIVE_INFINITY){
			return fa._totalScore;
		}
		fa._totalScore = 0.0;
		if (fa._fb._version != version){
			fa._fb._currScore = NetworkConfig.USE_FEATURE_VALUE ? fa.computeScore(param, fa.getCurrent(), fa._fb._fv) :
				fa.computeScore(param, fa.getCurrent());
			fa._fb._version = version;
		}
		fa._totalScore += fa._fb._currScore;
		/*
		if (fa._next != null){
			fa._totalScore += fa._next.getScore(param, version);
		}*/
		return fa._totalScore;
