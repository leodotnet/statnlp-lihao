...
public abstract class NetworkModel implements Serializable{
	...
	//public GlobalNetworkParam otherGNP = null;
	...
	public Instance[][] splitInstancesForTest(Instance[] testInsts) {
		...
		if (org.statnlp.commons.Utils.DEBUG)
	...
	public void initWeightFromOther() {
		
		if (EntityRelationGlobal.INIT_WEIGHT_FROM_LIGHT_MODEL) {
			for(String filename_model : EntityRelationGlobal.INIT_WEIGHT_FROM_OTHERS) {
				
				try {
					ObjectInputStream in = new ObjectInputStream(new FileInputStream(filename_model));
					double[] otherWeights = (double[]) in.readObject();
					System.err.println("Init Weight from model " + filename_model + " with #weight:" + otherWeights.length);
					
					double[] weights = this.getFeatureManager().getParam_G().getWeights();
					
					System.arraycopy(otherWeights, 0, weights, 0, otherWeights.length);
					in.close();
					
					System.err.println("Total #weight initialized:" + otherWeights.length + "\n");
		...
		GlobalNetworkParam MHgnp = null;
		
		if (EntityRelationGlobal.INIT_WEIGHT_FROM_OTHERS != null)
		for(String filename_model : EntityRelationGlobal.INIT_WEIGHT_FROM_OTHERS) {
				
			
			try {
				ObjectInputStream in = null;
				in = new ObjectInputStream(new FileInputStream(filename_model));
				NetworkModel MHmodel = (NetworkModel) in.readObject();
				MHgnp = MHmodel.getFeatureManager().getParam_G();
				
				this.getFeatureManager().getParam_G().setStoreFeatureReps();
				in.close();
			} catch (IOException | ClassNotFoundException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			...
			if (MHgnp == null) {
				System.err.println("Fail to load weight from model " + filename_model);
				continue;
			}
			
			int count = 0;
			
			GlobalNetworkParam gnp = this.getFeatureManager().getParam_G();
			
			MHgnp.getStringIndex().buildReverseIndex();
			StringIndex si = MHgnp.getStringIndex();
//				boolean[] weights = new boolean[gnp.getWeights().length];
//				Arrays.fill(weights, false);
			double[] otherWeights = MHgnp.getWeights();
			System.err.println("Init Weight from model " + filename_model + " with #weight:" + otherWeights.length);
			
			for(int type: MHgnp.getFeatureIntMap().keys()){
				TIntObjectHashMap<TIntIntHashMap> output2input = MHgnp.getFeatureIntMap().get(type);
				for(int output: output2input.keys()){
					...
					for(int input: input2id.keys()){
						...
						double otherWeight = otherWeights[id];
						int[] fIdx = new int[]{type, output, input};
						String[] f = new String[fIdx.length];
						for(int k = 0; k < f.length; k++) {
							f[k] = si.get(fIdx[k]);
						}
						
						int fId = gnp.getFeatureId(f[0], f[1], f[2]);
						if (fId >= 0) {
							gnp.setWeight(fId, otherWeight);
							count++;
						} else {
							//System.out.println("not matched feature:" + Arrays.toString(f));
						}
						
					}
				}
			}
			System.err.println("Total #weight initialized:" + count + "\n");
			
		}
			/*
			gnp.getStringIndex().buildReverseIndex();
			si = gnp.getStringIndex();
			for(int i = 0; i < weights.length; i++) {
				if (!weights[i]) {
					int[] fIdx = gnp.getFeatureRep(i);
					String[] f = new String[fIdx.length];
					for(int k = 0; k < f.length; k++) {
						f[k] = si.get(fIdx[k]);
					}
