...
public class NegationFocusSemiOIV5Compiler extends NegationCompiler {
	...
	public NegationFocusSemiOIV5Compiler() {
		super();
	}
	...
	public boolean satisfyConstraint(int tag_id, int next_tag_id) {
		...
		//String[] labelForms = new String[] { "O", "OR", "IR", "OVR", "IVR" };
	...
	public Network compileUnlabeledPruning(int networkId, Instance instance, LocalNetworkParam param) {
		...
		int[] tag_id_set = new int[] { 0, 1, 2, 3, 4 };
		int[] next_tag_id_set = new int[] { 0, 1, 2, 3, 4 };
		...
		for (int i = 0; i < inst.outputSemi.size(); i++) {
			...
			String currRoleName = currRoleIdx >= 0 ? sr.roleNameList.get(currRoleIdx) : "*";
			
			if (currRoleIdx < 0) {
				tag_id_set = new int[] { 0 };
			} else {
				if (currRoleName.startsWith("V")) {
					tag_id_set = new int[] { 3, 4 };
				} else 
					tag_id_set = new int[] { 1, 2 };
			}

			String nextRoleName = nextRoleIdx >= 0 ? sr.roleNameList.get(nextRoleIdx) : "*";
	...
	public ArrayList<Label> convert2Output(NegationInstance inst) {
		...
		while (true) {
			...
			if (roleIdx == -1) {
				...
				int tag_id = inst.focusScope[startIdx] + 1;
