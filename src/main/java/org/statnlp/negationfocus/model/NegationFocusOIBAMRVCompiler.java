...
public class NegationFocusOIBAMRVCompiler extends NegationCompiler {
	...
	public NegationFocusOIBAMRVCompiler() {
		super();
	}
		...
		for (int pos = 1; pos < size; pos++) {
			for (int last_tag_id = 0; last_tag_id < this._labels.length; last_tag_id++) {
				for (int tag_id = 0; tag_id < this._labels.length; tag_id++) {
					...
					if (last_tag_id == 1) {
						if (tag_id == 0 || tag_id == 5 || tag_id == 6) continue;
					}
					
					if (last_tag_id == 5 || last_tag_id == 6) {
	...
	public Label[] getLabels() {
		String[] labelForms = new String[] { "OB", "OA", "OM", "OR", "OV", "IR", "IV" };
