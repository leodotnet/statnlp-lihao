...
public class NegationScopeSemiCompact4Viewer extends VisualizationViewerEngine {
	...
	public NegationScopeSemiCompact4Viewer(Map<Integer, Label> labels){
		super(null);
		this.labels = labels;
	}
	
	
	public NegationScopeSemiCompact4Viewer(InstanceParser instanceParser) {
	...
	protected String label_mapping(VNode node) {
		...
		//int tag_id = ids[1];
		//int numSpan = NegationGlobal.MAX_NUM_SPAN - ids[2];
		int nodeType = ids[3];
		...
		if (nodeType == NodeType.O.ordinal()) {
			...
			label = NodeType.values()[nodeType].name();//Arrays.toString(ids);//NegationCompiler.LABELS[tag_id].getForm();
		}
