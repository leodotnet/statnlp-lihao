...
public class NegationSpanSemiOILatent2FeatureManager extends NegationFeatureManager {
	...
	public NegationSpanSemiOILatent2FeatureManager(GlobalNetworkParam param_g) {
		super(param_g);
	}

	public NegationSpanSemiOILatent2FeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		if (nodetype_parent == NodeType.Root.ordinal()) {
			...
			if (ct.startsWith("O")) {
				...
				for(String type : new String[] {"", latentTag}) {//latentTag
				
					fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_parent - 1, "", "", "scope_begin_-1" + type));
					fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_parent, "", "", "scope_begin_0" + type));
					
					fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_parent + 1, "", "", "scope_begin_+1" + type));
					
					fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_child - 2, "", "", "scope_end_-1" + type));
					fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_child - 1, "", "", "scope_end_0" + type));
					
					fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_child, "", "", "scope_end_+1" + type));
				}
