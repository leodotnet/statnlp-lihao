...
public class NegationScopeSemi4Viewer extends VisualizationViewerEngine {
	...
	public NegationScopeSemi4Viewer(Map<Integer, Label> labels){
		super(null);
		this.labels = labels;
	}
	
	
	public NegationScopeSemi4Viewer(InstanceParser instanceParser) {
	...
	protected String label_mapping(VNode node) {
		...
		if (nodeType == NodeType.Node.ordinal()) {
			if (tag_id == 0) {
				...
				label = Arrays.toString(ids);//NegationCompiler.LABELS[tag_id].getForm();
	...
	protected void initNodeCoordinate(VisualizeGraph vg)
	{
		for(VNode node : vg.getNodes())
		{
			...
			double y = mappedId * span_height + offset_height - 70 + numSpan * 30;
