...
public class NegationCueOICompiler2 extends NegationCompiler {
	...
	public NegationInstance decompile(Network network) {
		...
		for(int i = 0; i < predication_array.size(); i++) {
			...
			if (token.equals("injustice")) {
				System.out.println();
			}
			
			if (label.getForm().startsWith("I")) {
				inst.negation.cue[i] = 1;
				inst.negation.cueForm[i] = getCueForm(token);
				hasNegation = true;
			} else {
		...
		if (!hasNegation) {
			...
			for (int i = 0; i < predication_array.size(); i++) {
				...
				if (NegationGlobal.NegExpList.contains(phrase1)) {
					inst.negation.cue[i] = 1;
					...
					if (pos_tag.startsWith("A") || pos_tag.startsWith("N") || pos_tag.startsWith("V")) {
