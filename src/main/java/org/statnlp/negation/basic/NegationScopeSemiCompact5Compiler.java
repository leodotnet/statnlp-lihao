...
public class NegationScopeSemiCompact5Compiler extends NegationCompiler {
	...
	public NegationScopeSemiCompact5Compiler() {
		super();
	...
	protected long toNode_I4(int pos) {
		return NetworkIDMapper.toHybridNodeID(new int[] { 1000 - pos, 0, 0, NodeType.I4.ordinal()});
	}
	...
	public NegationInstance decompile(Network network) {
		...
		while (true) {
			...
			
			{
				...
				//edge: I3 O I4
				else if (nodetype_parent == NodeType.I3.ordinal() && nodetype_child == NodeType.O.ordinal() && nodetype_child2 == NodeType.I4.ordinal()) {
					for(int i = pos_parent; i < pos_child; i++) {
						span[i] = 1;
					}
				...
				//edge: I4 O X
				else if (nodetype_parent == NodeType.I4.ordinal() && nodetype_child == NodeType.O.ordinal() && nodetype_child2 == NodeType.X.ordinal()) {
					for(int i = pos_parent; i < pos_child; i++) {
						span[i] = 1;
					}
				}
	...
	public Network compileUnlabeled(int networkId, Instance instance, LocalNetworkParam param) {
		...
		for(int k = 0; k<size; k++) {
			...
			long node_I4 = this.toNode_I4(k);
			nsnetwork.addNode(node_I4);
		}
		...
		for(int pos = 0; pos<size; pos++) {
			...
			for(int L = 1; L+pos < size && L< L_max; L++) {
				long node_O = this.toNode_O(pos+L);
				for(int M = 1; L + M + pos < size && M < M_max; M++) {
					long node_I4 = this.toNode_I4(pos+L+M);
					nsnetwork.addEdge(node_I3, new long[] {node_O, node_I4});
				}
		...
		//edges for I4
		for(int pos = 0; pos<size; pos++) {
			long node_I4 = this.toNode_I4(pos);
			
			for(int L = 1; L+pos < size && L< L_max; L++) {
				long node_O = this.toNode_O(pos+L);
				nsnetwork.addEdge(node_I4, new long[] {node_O, node_X});
			}
		}
		...
		for(int k = 0; k<size; k++) {
			...
			if(nsnetwork.getChildren_tmp(node_I4)==null) {
				nsnetwork.addEdge(node_I4, new long[] {node_Y});
			}
	...
	public Network compileLabeled(int networkId, Instance instance, LocalNetworkParam param) {
		...
		if (numSpan == 0) {
			...
			int[] span4 = spans.get(3); //third span
			int beginIdx4 = span4[0];
			int endIdx4 = span4[1]; //exclusive
			...
			//I2 => I3
			long node_I4 = this.toNode_I4(beginIdx4);
			node_O = this.toNode_O(endIdx3);
			nsnetwork.addEdge(node_I3, new long[] {node_O, node_I4});
			
			//I3 => I4
			node_O = this.toNode_O(endIdx4);
			nsnetwork.addEdge(node_I4, new long[] {node_O, node_X});
			
			
		}else {
		
			System.err.println("spans.size() >= 4");
			System.exit(-1);
		}
