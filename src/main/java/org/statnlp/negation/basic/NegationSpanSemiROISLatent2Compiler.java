...
public class NegationSpanSemiROISLatent2Compiler extends NegationCompiler {
	...
	public NegationSpanSemiROISLatent2Compiler() {
		super();
		...
		for (int pos = 0; pos < size; pos++) {
			...
			if (lastTagId == 1 || (lastTagId == 0 && tagId == 1)) {
				...
				if (lastPos == -1) {
					...
					if (lastTagId == 0) {
						fromSet.add(node_array[lastPos][lastTagId][0]);
					} else { // (lastTagId == 1) 
						for(int latentId = 0; latentId < NegationGlobal.MAX_LATENT_NUMBER; latentId++) {
							fromSet.add(node_array[lastPos][lastTagId][latentId]);
						}
					}
				}
				...
				for(long from : fromSet) {
					if (tagId == 0) {
						long to = node_array[pos][tagId][0];
