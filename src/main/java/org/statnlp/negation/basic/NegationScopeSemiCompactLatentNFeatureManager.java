...
import org.statnlp.negation.basic.NegationScopeSemiCompactLatentNCompiler.*;

public class NegationScopeSemiCompactLatentNFeatureManager extends NegationFeatureManager {

	public NegationScopeSemiCompactLatentNFeatureManager(GlobalNetworkParam param_g) {
		super(param_g);
	}

	public NegationScopeSemiCompactLatentNFeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		int numspan_parent = 100 - ids_parent[1];
		int numspan_child = 100 - ids_child[1];
		
		int latentIdx_parent = 100 - ids_parent[2];
		int latentIdx_child = 100 - ids_child[2];
		...
		if (nodetype_child == NodeType.Y.ordinal()) {
			...
			int numspan_child2 = 100 - ids_child2[1];
			...
			//edge: I O (I, X)
				...
				int Iidx = numspan_parent;
				
				featureList.add(this._param_g.toFeature(network, "transition_", "O-I" + Iidx, "I" + Iidx));
				for(int i = pos_parent; i < pos_child; i++) {
					String nt = (i == pos_child - 1 ? "O" : "I" + Iidx);
					featureList.add(this._param_g.toFeature(network, "transition_", "I"  + Iidx, "I" + Iidx +"=>" + nt));
					fp = fp.addNext(this.getFeatureAtPos(network, inst, i, "I" + Iidx, nt));
				}
				...
				if (nodetypeChild2.startsWith("I")) {
					...
					for(int i = pos_child; i < pos_child2; i++) {
						...
						nt = (i == pos_child2 - 1 ? "I" + InextIdx : "O");
						featureList.add(this._param_g.toFeature(network, "transition_", "O", "O=>" + nt));
						fp = fp.addNext(this.getFeatureAtPos(network, inst, i, "O", nt));
					...
					String latentOutput = "I" + numspan_parent +  "-" + latentIdx_parent + "=>I" +  numspan_child2 +  "-" + latentIdx_child2;
