...
public class NegationCueOBMAICompiler4 extends NegationCompiler {
	...
	public NegationCueOBMAICompiler4() {
		super();
	}
	...
	public NegationInstance decompile(Network network) {
		...
		while (true) {
			...
			String tagStr = this._labels[parent_tag_id].getForm();
			String nodeTypeStr = NodeType.values()[parent_node_type].name();
	...
	public boolean satisfyConstraint(int tag_id, int next_tag_id) {
		...
		//String[] labelForms = new String[] { "OB", "OM", "OA", "I"};
		
		if (tag_id == 0 && (next_tag_id  == 1 || next_tag_id == 2))
			return false;
		
		if (tag_id == 1 && (next_tag_id == 0 || next_tag_id == 2))
			return false;
		
		if (tag_id == 2 && (next_tag_id == 0 || next_tag_id == 1))
			return false;
		
		if (tag_id == 3 && (next_tag_id == 0))
			return false;
		...
		for (int pos = 1; pos < size; pos++) {
			...
			for (int last_tag_id : last_tag_id_set) {
				for (int tag_id : tag_id_set) {
					...
					if (!satisfyConstraint(last_tag_id, tag_id)) continue;
