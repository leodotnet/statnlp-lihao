...
public class NegationSpanOIBAN6LatentICompiler extends NegationCompiler {
	...
	public NegationSpanOIBAN6LatentICompiler() {
		super();
		...
		for (int pos = 1; pos < size; pos++) {
			...
			for (int last_tag_id : last_tag_id_set) {
				for(int lastLatentId = 0; lastLatentId < NegationGlobal.MAX_LATENT_NUMBER; lastLatentId++) {
					...
					if (last_tag_id == 0 || last_tag_id == 4) break;
				}
			}
		}
	...
	public Network compileLabeled(int networkId, Instance instance, LocalNetworkParam param) {
		...
		for (int pos = 0; pos < size; pos++) {
			...
			if (tag_id != 0 && tag_id != 4) {
				...
				long to = node_array[pos][tag_id][0];
				toSet.add(to);
