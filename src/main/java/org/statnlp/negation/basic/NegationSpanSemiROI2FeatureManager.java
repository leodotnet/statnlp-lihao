...
public class NegationSpanSemiROI2FeatureManager extends NegationFeatureManager {
	...
	public NegationSpanSemiROI2FeatureManager(GlobalNetworkParam param_g) {
		super(param_g);
	}

	public NegationSpanSemiROI2FeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
		...
		if (nodetype_parent == NodeType.Root.ordinal()) {
			...
			if (ct.startsWith("I")) {
				...
				for(int pos = pos_parent; pos < pos_child; pos++) {
					...
					String nt = (pos == pos_child - 1) ? (pos < size ? "I" : "<END>") :  ct;
				...
				fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_parent - 1, "", "", "gap_begin_-1"));
				fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_parent, "", "", "gap_begin_0"));
				fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_parent + 1, "", "", "gap_begin_+1"));
				
				
				fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_child - 2, "", "", "gap_end_-1"));
				fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_child - 1, "", "", "gap_end_0"));
				fp = fp.addNext(this.getFeatureAtPos(network, inst, pos_child, "", "", "gap_end_+1"));
