...
import org.statnlp.negation.basic.NegationScopeSemiCompact4Compiler.*;

public class NegationScopeSemiCompact4FeatureManager extends NegationFeatureManager {

	public NegationScopeSemiCompact4FeatureManager(GlobalNetworkParam param_g) {
		super(param_g);
	}

	public NegationScopeSemiCompact4FeatureManager(GlobalNetworkParam param_g, String neuralType, boolean moreBinaryFeatures) {
		...
		if (nodetype_child == NodeType.Y.ordinal()) {
			...
			//edge: I O (I, X)
				...
				if (nodetypeChild2.startsWith("I")) {
					...
					featureList.add(this._param_g.toFeature(network, "transition_", "O-I", "I" + spannum_parent));
					featureList.add(this._param_g.toFeature(network, "transition_", "O-I", "I"));
					
					
					for(int i = pos_child; i < pos_child2; i++) {
						String nt = (i == pos_child2 - 1 ? "I" + spannum_child2 : "O");
						featureList.add(this._param_g.toFeature(network, "transition_", "O", "O=>" + nt));
						fp = fp.addNext(this.getFeatureAtPos(network, inst, i, "O", nt, "_gap_"));
						
						nt = (i == pos_child2 - 1 ? "I" : "O");
						featureList.add(this._param_g.toFeature(network, "transition_", "O", "O=>" + nt));
						fp = fp.addNext(this.getFeatureAtPos(network, inst, i, "O", nt, "_gap_"));
						
						String token = Utils.getToken(inputs, i, FEATURE_TYPES.word.ordinal()).toLowerCase();
						String POSTag = Utils.getToken(inputs, i, FEATURE_TYPES.pos_tag.ordinal());
						
						featureList.add(this._param_g.toFeature(network, "gap_", "", token));
						featureList.add(this._param_g.toFeature(network, "gap_", "", POSTag));
						
						if (POSTag.equals("CC")) {
							featureList.add(this._param_g.toFeature(network, "gap_", "contains_CC", ""));
						}
						
						if (token.equals("but")) {
							featureList.add(this._param_g.toFeature(network, "gap_", "contains_CC", ""));
							featureList.add(this._param_g.toFeature(network, "gap_", "contains_CC", "but"));
						}
						
						if (inst.negation.cue[i] == 1) {
							featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue", ""));
							featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue", inst.negation.cueForm[i]));
							
							if (i == pos_child) {
								featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue_firsttoken", ""));
								featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue_firsttoken", inst.negation.cueForm[i]));
								
							}
							
							if (i == pos_child2 - 1) {
								featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue_lasttoken", ""));
								featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue_lastttoken", inst.negation.cueForm[i]));
					...
					//cue is the first token of next partial scope
					if (inst.negation.cue[pos_child2] == 1) {
						
						
						featureList.add(this._param_g.toFeature(network, "I", "contains_cue", ""));
						featureList.add(this._param_g.toFeature(network, "I", "contains_cue", inst.negation.cueForm[pos_child2]));
						
						
						featureList.add(this._param_g.toFeature(network, "I", "contains_cue_firsttoken", ""));
						featureList.add(this._param_g.toFeature(network, "I", "contains_cue_firsttoken", inst.negation.cueForm[pos_child2]));
					...
					for(int i = pos_child; i < size; i++) {
						...
						if (inst.negation.cue[i] == 1) {
							featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue", ""));
							featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue", token));
							
							if (i == pos_child) {
								featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue_firsttoken", ""));
								featureList.add(this._param_g.toFeature(network, "gap_", "contains_cue_firsttoken", token));
					...
					System.err.println("Unexpected cases");
					System.exit(-1);
				}
			}
