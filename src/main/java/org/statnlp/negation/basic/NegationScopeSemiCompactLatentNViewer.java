...
import org.statnlp.negation.basic.NegationScopeSemiCompactNCompiler;
import org.statnlp.negation.basic.NegationScopeSemiCompactNCompiler.*;
...
public class NegationScopeSemiCompactLatentNViewer extends VisualizationViewerEngine {
	...
	public NegationScopeSemiCompactLatentNViewer(Map<Integer, Label> labels){
		super(null);
		this.labels = labels;
	}
	
	
	public NegationScopeSemiCompactLatentNViewer(InstanceParser instanceParser) {
	...
	protected String label_mapping(VNode node) {
		...
		//int tag_id = ids[1];
		int numSpan = 100 - ids[1];
		int latentIdx = 100 - ids[2];
		int nodeType = ids[3];
	...
	protected void initNodeCoordinate(VisualizeGraph vg)
	{
		for(VNode node : vg.getNodes())
		{
			...
			if(nodeType == NodeType.Root.ordinal()){
				...
				y = (mappedId + numSpan + 1) * span_height + offset_height - 30 + 10 * latentIdx;
			}
