...
public class MentionHypergraphFeatureManager extends FeatureManager {
	...
	protected FeatureArray extract_helper(Network net, int parent_k, int[] children_k, int children_k_index) {
		...
		String labelId = Label.LABELS_INDEX.get(parent_arr[2]).form;//String.valueOf(parent_arr[2]);
		
		if(nodeType == NodeType.X || nodeType == NodeType.A || nodeType == NodeType.E){
		...
		for(int child_k: children_k){
			...
			if(nodeType == NodeType.T && childNodeType == NodeType.I){
