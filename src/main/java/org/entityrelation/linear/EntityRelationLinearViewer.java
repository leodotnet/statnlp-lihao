...
public class EntityRelationLinearViewer extends VisualizationViewerEngine {
	...
	public EntityRelationLinearViewer(Map<Integer, Label> labels){
		super(null);
		this.labels = labels;
	}
	
	
	public EntityRelationLinearViewer(InstanceParser instanceParser) {
	...
	protected void initNodeCoordinate(VisualizeGraph vg)
	{
		for(VNode node : vg.getNodes())
		{
			...
			if(nodeType == NodeType.Root.ordinal()){
				...
				//x += 0;//+= (size ) * span_width ;
				y += (size + 1) * span_height;
			} else if (nodeType == NodeType.T.ordinal()){
				x += (tripleIdx * 2 + size) * span_width + 25 * r + 200;
				y += (size * 2) * span_height + 10 * r ;
			} else if (nodeType == NodeType.I1.ordinal()) {
				x += tripleIdx * 2 * span_width + span_width + span_width + 25 * r + 25 * i;
				y += (size * 3) * span_height + span_height + 25 * r + 25 * k;
			} else if (nodeType == NodeType.I2.ordinal()) {
				x += tripleIdx * 2 * span_width + span_width + span_width + 25 * r + 25 * i - 5;
				y += (size * 3) * span_height + span_height + 25 * r + 25 * k -5;
			}
