...
public class EntityRelationSemiBTCompiler extends EntityRelationCompiler {
	...
	public EntityRelationSemiBTCompiler() {
		super();
	}
	...
	public EntityRelationInstance decompile(Network network) {
		...
		while (!queue.isEmpty()) {
			...
			if (nodetype_parent == NodeType.Root.ordinal()) {
				...
				int j = size - child1_ids[0];
				
				Entity entity1 = new Entity(new int[] {i, j + 1}, new int[]{-1, -1}, EntityRelationOutput.getENTITY(t1));
				int p1 = EntityRelationOutput.getEntityIdx(entities, entity1);
				if (p1 < 0) {
					entities.add(entity1);
					p1 = entities.size();
				} else {
					entity1 = entities.get(p1);
				}
				...
				int l = size - child2_ids[0];
				
				Entity entity2 = new Entity(new int[] {k, l + 1}, new int[]{-1, -1}, EntityRelationOutput.getENTITY(t1));
				int p2 = EntityRelationOutput.getEntityIdx(entities, entity2);
				if (p2 < 0) {
					entities.add(entity2);
					p2 = entities.size();
				} else {
					entity2 = entities.get(p2);
				}
		...
		for(int i = 0; i < size; i++) {
			for(int k = 0; k < size; k++) {
				...
				for(int pTriple = 0; pTriple < T.length; pTriple++) {
					...
					if (EntityRelationGlobal.ADD_SELF_RELATION && r == 0) { //self-relation
						for(int l1 = 1; l1 < L_MAX && i + l1 < size && k + l1 < size; l1++) {
							long node_I1 = this.toNode_I1(size, i + l1, k, r, t1, t2);
							long node_I2 = this.toNode_I2(size, i, k + l1, r, t1, t2);
						...
						for(int l1 = 1; l1 < L_MAX && i + l1 < size; l1++) {
							for(int l2 = 1; l2 < L_MAX && k + l2 < size; l2++) {
								long node_I1 = this.toNode_I1(size, i + l1, k, r, t1, t2);
								long node_I2 = this.toNode_I2(size, i, k + l2, r, t1, t2);
				...
				for(int pTriple = 0; pTriple < T.length; pTriple++) {
					...
					if (relationsCurr.size() == 0) {
						...
						for(int pRel = 0; pRel < relationsCurr.size(); pRel ++) {
							...
							long node_I1 = this.toNode_I1(size, j, k, r, t1, t2);
							long node_I2 = this.toNode_I2(size, i, l, r, t1, t2);
