...
public class CitationSpanSemiCompactNViewer extends VisualizationViewerEngine {
	 ...
	 */
	protected CitationSpanInstance<Integer> instance;
	...
	public CitationSpanSemiCompactNViewer(Map<Integer, Label> labels){
		super(null);
		this.labels = labels;
	}
	
	
	public CitationSpanSemiCompactNViewer(InstanceParser instanceParser) {
	...
	{
		this.instance = (CitationSpanInstance<Integer>)super.instance;
	...
	protected String label_mapping(VNode node) {
		...
		if (nodeType == NodeType.O.ordinal()) {
			label = pos < size ? this.inputs.get(pos)[CitationSpanInstance.FEATURE_TYPES.word.ordinal()] : "<END>";
...
//					if (pos < size && pos == this.instance.cite_id_pos[0])
//						node.color = colorMap[8];
				}
				else
				{
	...
	protected void initNodeCoordinate(VisualizeGraph vg)
	{
		for(VNode node : vg.getNodes())
		{
			...
			if(nodeType == NodeType.Root.ordinal()){
				...
				y = (mappedId + numSpan + 1) * span_height + offset_height;
			}
