...
public class CitationSpanMetric implements Metric {
	...
	public CitationSpanMetric(boolean aggregation) {
	...
	public void aggregate(CitationSpanMetric metric) {
	...
	public boolean isBetter(Metric other) {
		CitationSpanMetric metric = (CitationSpanMetric)other;
		
		boolean better = false;
		
		if (BESTON == EVALOPTION.Exact) {
			if (metric.spanExactStatPunc[2] == Double.NaN)
			metric.spanExactStatPunc[2] = 0.0;
			better = Math.abs(spanExactStatPunc[2] - metric.spanExactStatPunc[2]) > 1e-5 ? spanExactStatPunc[2] > metric.spanExactStatPunc[2] : spanTokenStatPunc[2] > metric.spanTokenStatPunc[2];
		} else if (BESTON == EVALOPTION.Token) {
			if (metric.spanTokenStat[2] == Double.NaN)
				metric.spanTokenStat[2] = 0.0;
			
			better = Math.abs(spanTokenStatPunc[2] - metric.spanTokenStatPunc[2]) > 1e-5 ? spanTokenStatPunc[2] > metric.spanTokenStatPunc[2] : spanExactStatPunc[2] > metric.spanExactStatPunc[2];
	...
	public String getMetricValue() {
		return this.spanExactStatPunc[2] + "\t\t" + spanTokenStatPunc[2] + "\t\t" + spanAcc;
	}
	...
	public CitationSpanMetric compute() {
